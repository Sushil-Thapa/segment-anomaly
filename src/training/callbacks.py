...
class EarlyStopping:
    ...
    def __call__(
        self, epoch: int, metrics: Dict[str, float], model: torch.nn.Module
    ) -> bool:
        """
        ...
        if self.wait >= self.patience:
            ...
            logger.info(
                f"Early stopping at epoch {epoch} with {self.monitor}={self.best_value:.4f}"
            )
            return True
    ...
    def __call__(
        ...
        if should_save:
            ...
            filepath = self.filepath.format(
                epoch=epoch, **{key: value for key, value in metrics.items()}
            )
            ...
            if self.verbose:
                logger.info(
                    f"Saved checkpoint: {filepath} ({self.monitor}={current_value:.4f})"
                )
            ...
            if len(self.best_values) > self.save_top_k:
                ...
                try:
                    ...
                    if self.verbose:
                        logger.info(
                            f"Removed old checkpoint: {old_file} ({self.monitor}={old_value:.4f})"
                        )
            ...
            if num_pos == 0:
                ...
                if num_hard > 0 and num_neg > 0:
                    _, hard_indices = torch.topk(
                        loss_b[neg_mask], min(num_hard, num_neg)
                    )
...
class LearningRateScheduler:
    ...
    def __call__(
        self, epoch: int, metrics: Dict[str, float], optimizer: torch.optim.Optimizer
    ) -> None:
        """
            ...
            if "ReduceLROnPlateau" in str(type(self.scheduler)):
                ...
                if metric_value is not None:
                    ...
                    if self.verbose and old_lr != new_lr:
                        logger.info(
                            f"Reduced learning rate from {old_lr:.2e} to {new_lr:.2e}"
                        )
            else:
                self.scheduler.step()
...
def test_callbacks():
    ...
    checkpoint = ModelCheckpoint(
        filepath="test_epoch_{epoch:02d}_{val_loss:.4f}.pth", monitor="val_loss"
    )
