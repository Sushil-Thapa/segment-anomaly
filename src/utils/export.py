        ...
        if optimize:
            ...
            try:
                ...
                if check:
                    ...
                    logger.warning(
                        "ONNX model optimization failed, using unoptimized version"
                    )
...
def to_torchscript(
    ...
    try:
        ...
        with torch.no_grad():
            ...
            logger.info(
                f"TorchScript model verification passed, output shape: {output.shape}"
            )
...
def create_calibration_dataset(
    data_loader: torch.utils.data.DataLoader, num_batches: int = 100
) -> List[torch.Tensor]:
    """
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Export trained model to ONNX/TorchScript"
    )
    ...
    parser.add_argument(
        "--batch_size", type=int, default=1, help="Batch size for export"
    )
    parser.add_argument("--fp16", action="store_true", help="Use FP16 precision")
    parser.add_argument(
        "--optimize", action="store_true", default=True, help="Optimize exported model"
    )
    parser.add_argument(
        "--benchmark",
        action="store_true",
