...
class TiledInference:
    ...
    def predict_image(
        self, image: np.ndarray, return_probabilities: bool = False, min_area: int = 100
    ) -> Dict[str, Any]:
        """
        ...
        logger.info(
            f"Processing {len(tile_coords)} tiles of size {self.tile_size}x{self.tile_size}"
        )
        ...
        for i in range(0, len(tile_coords), self.batch_size):
            ...
            if (i // self.batch_size + 1) % 10 == 0:
                logger.info(
                    f"Processed {i + len(batch_coords)}/{len(tile_coords)} tiles"
                )
        ...
        pred_mask = self._postprocess_predictions(
            stitched_probs, original_shape, min_area
        )
    ...
    def predict_image_file(
        self, image_path: str, output_path: Optional[str] = None, **kwargs
    ) -> Dict[str, Any]:
        """
    ...
    def benchmark(
        self, image_shape: Tuple[int, int, int] = (2048, 2048, 3), num_runs: int = 5
    ) -> Dict[str, float]:
        """
        ...
        """
        logger.info(
            f"Benchmarking inference on {image_shape} image for {num_runs} runs"
        )
        ...
        stats = {
            ...
            "throughput_mpx_per_s": (image_shape[0] * image_shape[1] / 1e6)
            / np.mean(times),
        }
...
def create_inference_engine(
    checkpoint_path: str, config: Dict[str, Any], device: Optional[torch.device] = None
) -> TiledInference:
    """
...
def test_inference():
    ...
    # Create inference engine
    inference_engine = TiledInference(
        model=model, tile_size=256, stride=128, device=torch.device("cpu"), fp16=False
    )
